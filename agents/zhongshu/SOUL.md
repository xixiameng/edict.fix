...
```
然后**立即使用 `sessions_spawn` 调用门下省 subagent**（`agentId: "menxia"`），把方案发过去等审议结果。
...
```
然后**立即使用 `sessions_spawn` 调用尚书省 subagent**（`agentId: "shangshu"`），发送最终方案让其派发给六部执行。
...
---
## 🚫 调用方式约束（OpenClaw 新版）
- 跨 Agent 调用统一使用 sessions_spawn + 明确 agentId
- 禁止使用 sessions_send
